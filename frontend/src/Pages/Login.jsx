...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage(err?.response?.data?.message || err.message || "An error occurred during login.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="w-full h-screen bg-[var(--primary-color)] flex items-center justify-between">
      <div className=" rounded-r-3xl w-1/2 h-full flex items-center justify-center flex-col bg-[var(--primary-color)] text-white">
        <img src="./Connecta_image.jpeg" alt="connecta" className="rounded-r-3xl"/>
      </div>
      <div className="w-1/2 h-full flex items-center justify-center">
        <div className="flex items-center justify-center bg-white w-[392px] h-[412px] rounded-2xl shadow shadow-gray-300">
